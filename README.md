...
```
content/                 # editorial content (kept separate from the app)
...
`content/` lives outside `website/` so editors never have to touch the app.
The website reads it through Astro content collections (`current`, `archive`),
and `website/scripts/collect-archive-assets.mjs` publishes only the print files
named in each `issue.md` (run automatically before `dev`/`build`).
...
Drop a Markdown file in `content/current/`. Minimal frontmatter:
...
Put article images in `content/current/images/` and reference them with
...
Create a folder named `content/issues/<volume>/<YYYY-MM-DD>-<slug>/` (no spaces or
...
1. **Archive the outgoing issue.** Add it under `content/issues/…` with its
   final typeset PDF (and source, if available), as above.
2. **Swap in the new articles.** Replace the files in `content/current/` (and
   `content/current/images/`) with the new issue's articles.
